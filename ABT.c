...
int A_acknum = 1, A_next = -1, A_buf = 0;
int A_running = 0;
int A_free = 1;
...
void A_transmit()
{
	if(A_free == 1 && A_buf > A_next){
	A_next = ++A_next % 1000;
	A_buffer[A_next].packet.seqnum = ++A_acknum; //+= sizeof(packetA.payload);
	A_buffer[A_next].packet.acknum = 1;
	A_buffer[A_next].packet.checksum = computeChecksum(A_buffer[A_next].packet);
	A_buffer[A_next].acked = 0;
	//Packet load ends

	tolayer3(0, A_buffer[A_next].packet);
	A_transport++;

	if(A_running == 1)
		stoptimer(0);
	printf("Sending buffered packet %d", A_buffer[A_next].packet.seqnum);
  ...
  struct msg message;
{
	...
	strcpy(A_buffer[A_buf].packet.payload, message.data);
	A_buffer[A_buf].packet.payload[20] = '\0';
...
void A_input(packet)
  struct pkt packet;
{
	printf("\n Arriving packet %d", packet.acknum);

	if(packet.checksum == computeChecksum(packet) && packet.acknum  == A_buffer[A_next].packet.seqnum && A_buffer[A_next].acked == 0)
	{
		A_buffer[A_next].acked = 1;
		A_free = 1;
		stoptimer(0);
		A_running = 0;
...
void A_timerinterrupt()
{
	...
	A_transport++;
	starttimer(0, TIMEOUT);
	printf("\n Retransmitting packet %d", A_buffer[A_next].packet.seqnum);
	A_running = 1;
...
int B_acknum = 1; //variable to keep track of bytes received by B
...
printf("B: sequence %d Acknum : %d csum %d packet csum %d", packet.seqnum, B_acknum, cSum, packet.checksum);	
	if(cSum == packet.checksum && packet.seqnum == B_acknum + 1)
	{	//The packet has correct data; send to Layer 5
		packet.payload[20] = '\0';
		...
		//Load ACK packet
		packetB.seqnum = 1;
		packetB.acknum = ++B_acknum; //= packet.seqnum; //Packet sequence number will be updated everytime by the sender
		packetB.checksum = computeChecksum(packetB);
		//Load ended
	}
	else
	{
		//Load a duplicate ACK packet
		packetB.seqnum = 1;
		packetB.acknum = B_acknum;
		packetB.checksum = computeChecksum(packetB);
	}

	//printf("\n%d\t%d\t%d %s", packetB.seqnum, packetB.acknum, packetB.checksum, packet.payload);
