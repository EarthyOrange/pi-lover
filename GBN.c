...
int SND_BUFSIZE = 1000;
int RCV_BUFSIZE = 1; //Receiver's Buffer size

int A_base = 0, A_nextSeq = 0, A_seqnum = 1, A_bufSeq = 0;;

struct qdpkt A_buffer[1001];
  ...
  struct msg message;
{
	...
	strcpy(A_buffer[A_bufSeq % SND_BUFSIZE].packet.payload, message.data);
	A_buffer[A_bufSeq % SND_BUFSIZE].packet.payload[20] = '\0';
	A_bufSeq++;  

	if(A_bufSeq % SND_BUFSIZE == A_base % SND_BUFSIZE && A_bufSeq > A_base + WINSIZE)
	{
		printf("\nBuffer full.");
		exit(1);
	}
	...
	{
		int nextSeqIndex = A_nextSeq % SND_BUFSIZE;

		A_buffer[nextSeqIndex].packet.seqnum = A_seqnum += sizeof(A_buffer[nextSeqIndex].packet.payload);
		A_buffer[nextSeqIndex].packet.acknum = 1;
	//	strcpy(A_buffer[nextSeqIndex].packet.payload, message.data);
		A_buffer[nextSeqIndex].packet.payload[20] = '\0';
		A_buffer[nextSeqIndex].packet.checksum = computeChecksum(A_buffer[nextSeqIndex].packet);
		
		tolayer3(0, A_buffer[nextSeqIndex].packet);
		A_transport++;
	...
	{
		printf("%d ", A_buffer[i % SND_BUFSIZE].packet.seqnum);
	}
	printf("\n");
  ...
  struct pkt packet;
{
	...
	{
		int i = A_nextSeq - 1, j = 0, matched = 0;

		for(;i >= A_base;i--)
		{
			if(A_buffer[i % SND_BUFSIZE].packet.seqnum == packet.acknum)
			{
				j = i + 1;

				while(A_buffer[i % SND_BUFSIZE].acked != 1 && i >= A_base)
				{
  ...
  struct pkt packet;
{
	...
	if(packet.checksum == computeChecksum(packet) && packet.seqnum == B_acknum + sizeof(packet.payload))
	{
		packet.payload[20] = '\0';
		printf("\n%s", packet.payload);	
		...
		packetB.seqnum = 1;
		packetB.acknum = B_acknum = packet.seqnum;
